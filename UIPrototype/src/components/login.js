...
import { fetchHeaderAvatar } from "@/store/modules/user";
...
export const Login = () => {
	...
	useEffect(() => {
		dispatch(fetchHeaderAvatar(user_id));
	}, [dispatch, user_id])
	const headerAvatar = useSelector(state => state.user).headerAvatar
		...
		<Link to={{ pathname: `/user/${user_id}` }}>
			...
			>
				<span className=" relative top-0 right-5 ">
					<Avatar src={<img src={headerAvatar} alt="avatar" />} size={64} />
				</span>
			</div>
		</Link>
	)
