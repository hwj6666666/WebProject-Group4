...
const Object = ({ object }) => {
  ...
  const formattedScore = object && object.aveScore ? object.aveScore.toFixed(1) : '';
      ...
      <div className="flex flex-row mb-2">
        ...
        <div className="flex-grow">
          <div className="flex flex-row items-end justify-between">
            <div><h2 className="text-3xl mt-10 mr-10 font-bold">{object.title}</h2></div>
          ...
          <div className="flex flex-row items-start justify-between">
            <div><p className="mt-3 mr-10 text-gray-600 ">{truncateDescription(object.description, 90)}</p></div>
            <div className="flex text-4xl mr-8 mt-3 text-scorecolor ">
              <p className="">{formattedScore}</p>
              </div>
          </div>
        </div>
      </div>
      <div className="inline-blocks bg-gradient-to-r from-bgcolor to-white mb-5 ml-8 w-1/2" >
        <p className="text-fontcolor">"{truncateComment(object.hottestRemark, 25)}"</p>
        </div>
    </div>
  );
}
